...
// A real rendered image of the actual "The / Com'mon / People" logo mark
// (matching the live site's colours, rotation and layout pixel-for-pixel,
// generated once and checked in rather than an earlier attempt at faking
// it with coloured table cells, which rendered thin and unconvincing).
// Real width/height baked in so the aspect ratio is always correct however
// it's scaled below.
const BRAND_LOGO_PATH = path.join(__dirname, "assets", "brand-logo.png");
const BRAND_LOGO_SIZE = { width: 2200, height: 373 };
...
// The real "The / Com'mon / People" logo mark image (see BRAND_LOGO_PATH
// above) — a first attempt at recreating this with coloured table cells
// came out thin and unconvincing, so this uses an actual rendered image of
// the real mark instead. Falls back to nothing (not a broken image icon)
// if the asset is ever missing.
function brandMark() {
  if (!fs.existsSync(BRAND_LOGO_PATH)) return new Paragraph({ text: "" });
  const displayWidth = 340;
  const displayHeight = Math.round(displayWidth * (BRAND_LOGO_SIZE.height / BRAND_LOGO_SIZE.width));
  return new Paragraph({
    alignment: AlignmentType.CENTER,
    children: [
      new ImageRun({
        data: fs.readFileSync(BRAND_LOGO_PATH),
