...
// Matched to The Com'mon People's actual visual identity (see public/style.css):
// bold, flat-coloured protest-poster/screen-print aesthetic — rotated stamp
// badges, thick outlines, condensed display type — not a soft corporate
// gradient. The image model was previously asked for "elegant minimalist
// gradients", which produced on-palette but off-brand results; this asks for
// the actual look instead.
async function coverArt({ companyName, jobDescription }) {
  const theme = topKeywords(jobDescription, 8).slice(0, 4).join(", ");
  const prompt =
    `A bold, punchy screen-printed protest-poster / union-rally style illustration for a premium personalised ` +
    `career document cover. Flat, high-contrast colour blocks with visible screen-print texture and grain — ` +
    `NOT soft gradients or airbrushed shading. Thick confident outlines, slightly rough/hand-cut poster-collage ` +
    `edges, a stamped rubber-badge or ticket-stub motif tilted at a jaunty angle somewhere in the composition. ` +
    `Colour palette, used as flat blocks: deep navy (#161F29) as the dominant background, warm mustard gold ` +
    `(#E0B03C), sky blue (#5AA9C2) and burnt orange (#D2691E) as bold accent shapes, with warm off-white paper ` +
    `cream (#ECE6D8) for highlights. Confident, working-class, grassroots-solidarity energy — think classic ` +
    `trade-union or gig-poster print design, not corporate stock art. Loosely thematic to a career in: ` +
    `${theme || "professional services"}. Absolutely no text, letters, numbers, logos, real company branding, ` +
    `recognisable buildings, or human faces — pure abstract/conceptual poster art, high-end print quality.`;
...
//
// IMPORTANT: these used to be 6, then 2, separate Claude calls. Live
// testing on Neil's actual account showed that even after fixing token
// limits and retry backoff, the two web-search calls (company research,
// market/challenges) kept failing while plain-text calls (gap analysis,
// pitch) succeeded reliably — every time. That specific pattern points at
// something Anthropic's own docs confirm: the web_search tool has its own
// separate rate-limit pool, distinct from the model's normal token/request
// limits. Two separate calls both using web_search meant drawing on that
// pool twice per report — on a low-tier account that's plausibly enough by
// itself, regardless of how generous the backoff is. The fix is to only
// need the web_search tool ONCE per report: all six research sections are
// now gathered in a single combined call instead of two.
// ---------------------------------------------------------------------
...
// All six research sections (company overview, recent news, employee
// sentiment, social media, market & sector intelligence, role challenges)
// in a single web-search call — see note above on why this was merged
// from two calls into one.
async function researchBundle(companyName, jobDescription) {
  const fallbacks = {
  ...
  const result = await claudeJSON(
    ...
    `for current, real information and cover SIX distinct angles as six separate sections:\n\n` +
    `1. COMPANY OVERVIEW (write 6-8 bullets): what they do and who they serve specifically; their size and ` +
    ...
    `2. RECENT NEWS (write 4-6 bullets): recent news and press coverage from roughly the last 6-12 months — ` +
    ...
    `3. EMPLOYEE SENTIMENT (write 4-6 bullets): what current and former employees say — Glassdoor, Google ` +
    ...
    `4. SOCIAL MEDIA (write 4-6 bullets): their social media presence — LinkedIn, Instagram, X/Twitter, ` +
    ...
    `5. MARKET & SECTOR INTELLIGENCE (write 4-6 bullets): current market and sector trends relevant to this ` +
    `company and role (see job description below) — economic pressure on the sector, regulatory change, ` +
    `technology shift, consumer/customer behaviour change, competitive dynamics — and how that context ` +
    `might shape what this interviewer actually cares about.\n\n` +
    `6. ROLE CHALLENGES (write 4-6 bullets): the real, current challenges someone would likely face in this ` +
    ...
    `pressure, talent retention/competition for this type of role, or anything specific to this company. Be ` +
    `realistic, not alarmist.\n\n` +
    `JOB DESCRIPTION (relevant to sections 5 and 6):\n${jobDescription || "(not provided)"}\n\n` +
    `Every bullet must be genuinely specific and concrete — names, numbers, dates, real detail — never vague ` +
    `filler like "they seem to value teamwork". If you genuinely can't find reliable information for a ` +
    `section, say so plainly rather than guessing or padding. Never invent facts about a real company.\n\n` +
    `Also return a "citedSources" array listing every distinct real webpage you actually drew on across all ` +
    `six sections above — every item needs a genuine, real URL you found via web search (never invent or ` +
    `guess a URL). Aim for at least 6-10 if your research genuinely covered that many distinct pages.\n\n` +
    ...
    `"bullets": [...]}, "socialMedia": {"headline": "...", "bullets": [...]}, "marketIntelligence": ` +
    `{"headline": "...", "bullets": [...]}, "roleChallenges": {"headline": "...", "bullets": [...]}, ` +
    `"citedSources": [{"title": "...", "url": "https://..."}, ...]}`,
    { webSearch: true, maxTokens: 8000 }
  );

  if (!result) return fallbacks;
  // Prefer the model's own self-reported source list (reliable — it's just
  // JSON text) merged with whatever the API's automatic citation metadata
  // attached (see claudeText in lib/aiClients.js) — in practice the
  // automatic mechanism often attaches nothing at all when the whole
  // response is one JSON blob rather than natural prose, so the model
  // self-reporting its sources directly is the more dependable path. Both
  // are kept and de-duped by URL in case either one alone misses something.
  const citationSources = Array.isArray(result.sources) ? result.sources : [];
  const modelReportedSources = Array.isArray(result.citedSources)
    ? result.citedSources.filter((s) => s && typeof s.url === "string" && s.url.startsWith("http"))
    : [];
  const seenUrls = new Set();
  const sources = [];
  [...citationSources, ...modelReportedSources].forEach((s) => {
    if (s.url && !seenUrls.has(s.url)) { seenUrls.add(s.url); sources.push({ url: s.url, title: s.title || s.url }); }
  });
  return {
...
// Fallback only — generic but still usable if the AI call fails. The real
// version (below) grounds every question in something specific from the
// job description, the way the STAR competency questions already do (see
// lib/questionBank.js); this used to be templated text only, which read as
// noticeably weaker than the rest of the report.
function questionsToAskHeuristic(companyName) {
...
async function questionsToAsk(companyName, jobDescription) {
  const fallback = questionsToAskHeuristic(companyName);
  if (!hasAnthropic() || !jobDescription || !jobDescription.trim()) return fallback;

  const result = await claudeJSON(
    `You're helping a candidate prepare thoughtful questions to ask THEIR interviewer at "${companyName || "this company"}", ` +
    `based on the specific job description below. Write questions that are grounded in real, specific details ` +
    `from the job description (a named responsibility, tool, team structure, challenge, or priority it mentions) ` +
    `— never generic template questions that could apply to any job. Each question should make the interviewer ` +
    `think "good question, they clearly read this properly."\n\n` +
    `JOB DESCRIPTION:\n${jobDescription}\n\n` +
    `Return JSON in exactly this shape, with 2 questions per category except the last (1 question):\n` +
    `{"Type 1 — About the company": ["...", "..."], "Type 2 — About the job": ["...", "..."], ` +
    `"Type 3 — About the interviewers": ["...", "..."], ` +
    `"Type 4 — About you, in this role": ["Based on our conversation so far, is there anything you'd like me to clarify about my fit for this role?"]}`,
    { maxTokens: 900 }
  );

  const categories = ["Type 1 — About the company", "Type 2 — About the job", "Type 3 — About the interviewers", "Type 4 — About you, in this role"];
  const valid = result && categories.every((c) => Array.isArray(result[c]) && result[c].length > 0);
...
async function generateReport(input) {
  ...
  const [researchResult, gaps, pitch, starAnswers, cover, questions] = await Promise.all([
    researchBundle(companyName, jobDescription),
    ...
    questionsToAsk(companyName, jobDescription),
  ]);

  const {
    research, recentNews: news, employeeSentiment: sentiment, socialMedia: social,
    marketIntelligence: market, roleChallenges: challenges,
  } = researchResult;
