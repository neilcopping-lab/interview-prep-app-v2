...
async function claudeText(prompt, { webSearch = false, maxTokens = 1024 } = {}) {
  ...
  try {
    ...
    // queued behind it then hits 429s immediately — and with only ~5.6s of
    // total backoff (the old 800/1600/3200ms schedule), those calls gave
    // up long before the per-minute window actually reset, which is what
    // caused market intelligence, role challenges and gap analysis to
    // silently fall back even though nothing was actually broken, just
    // rate-limited. Anthropic's per-minute limits reset on a rolling
    // ~60s window, so the backoff needs to plausibly span that, not a
    // few seconds.
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        ...
        // 3s, 6s, 12s, 24s, 48s, 96s (~189s total) — a genuinely low-tier
        // account's per-minute window may need more than one reset cycle
        // to recover from a large web-search call ahead of it in the
        // queue. This makes worst-case report generation slower, but a
        // slow, complete report beats a fast one with three blank
        // sections — and the frontend's countdown already degrades
        // gracefully into an open-ended "almost there" message past its
        // estimate, so a long wait doesn't look broken to the user.
