  ...
  try {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        ...
        // 2s, 4s, 8s, 16s, 32s — deliberately long enough that a call
        // queued right behind a big, budget-exhausting request has a real
        // chance of landing after the per-minute window resets, rather
        // than burning through all its retries in the first few seconds.
