...
// £25 (or £70 with the coaching add-on) on this report, so the bar is
...
// rate limit.
// ---------------------------------------------------------------------
// UPDATE: the real root cause of the "calls randomly falling back, even a
// lone one with nothing running alongside it" pattern was found (see the
// `thinking: { type: "disabled" }` change in claudeText below) — it was
// claude-sonnet-5's adaptive-thinking mode silently burning the whole
// max_tokens budget on invisible reasoning before ever writing the actual
// answer, completely unrelated to concurrency. Every round of tuning this
// number was chasing the wrong variable. Now that the actual cause is
// fixed, there's no reason left to force everything serial — report
// generation fires 5 real Claude calls at once (see the Promise.all in
// reportGenerator.js's generateReport), so allowing all 5 to run together
// removes the artificial queueing delay that was slowing every report down
// for no real benefit. If a genuine account-level rate limit ever does get
// hit at this concurrency, the retry/backoff below still has room to
// recover — but that's now a real, separate problem to solve if it
// actually shows up, not something to pre-emptively serialise around.
