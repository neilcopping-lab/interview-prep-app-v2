...
// fixed that specific symptom. But going straight to full concurrency (4)
// was overcorrecting: a live test right after raising it showed 3 of 4
// report-generation calls firing in the exact same instant burst back to
// falling back, while the 4th (a smaller, lone call made earlier in the
// flow, not part of that burst) succeeded fine — which is the original
// "several calls hit the API at once and trip a real per-minute limit"
// problem this project started with, not a key problem. Both things were
// true at different points: a bad key caused one failure mode, and a
// genuinely modest account-level rate limit is a separate, real
// constraint underneath it. Settled on 2 as a middle ground — real
// overlap (meaningfully faster than fully serial) without slamming the
// API with every call at once.
...
async function claudeText(prompt, { webSearch = false, maxTokens = 1024 } = {}) {
  ...
  try {
    ...
    // was tuned assuming ALL past failures were the bad-key issue — turned
    // out a genuine, modest rate limit is also real (see the
    // MAX_CONCURRENT_CLAUDE_CALLS comment above), so retries still need
    // real room to recover from a burst of parallel calls, just not the
    // multi-minute extreme from before the key was fixed. 5 attempts,
    // topping out around 30s total, splits the difference.
