  ...
  for (let attempt = 1; attempt <= MAX_JSON_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      try {
        ...
        // Logging just err2.message ("Unexpected token...") has never once
        // told us WHAT the model actually sent, only that it didn't parse —
        // useless for telling a genuine model slip-up apart from, say, the
        // model wrapping the JSON in an explanatory paragraph. A capped
        // snippet of the actual text is what's needed to see the real shape
        // of the failure next time this fires in production.
        console.error(
          `[AI-DIAGNOSTIC][claudeJSON] attempt ${attempt} could not parse response${willRetry ? ", retrying with a fresh call" : ", falling back"} — ${err2.message}\n` +
