...
const QUESTION_OPENERS = [
  ...
  "what would you do if", "how would you decide", "how would you approach",
  "walk me through how you'd", "walk me through how you would",
  "what's your view on", "what is your view on", "how would you handle",
];
...
async function selectQuestions(jobDescription, count = 5) {
  ...
  const result = await claudeJSON(
    `You're a sharp, experienced senior recruiter writing competency-based interview questions for the role ` +
    `described below. Write ${count} questions that a genuinely good interviewer would ask for THIS specific ` +
    `role — the kind that makes a candidate think "they really read the job spec," not questions that could ` +
    `be pulled off a generic list of interview questions.\n\n` +
    `AVOID THE OBVIOUS: don't just restate a JD requirement as a fill-in-the-blank template (e.g. turning ` +
    `"manages a £500k budget" into "tell me about a time you managed a budget" is lazy and predictable — ` +
    `everyone asks that). Instead, dig one level deeper: ask about the judgement calls, trade-offs, or ` +
    `tensions the role actually involves. Good techniques: combine two distinct requirements into one sharper ` +
    `question (e.g. balancing budget ownership against agency management, or reconciling a stated value with ` +
    `a stated pressure); ask about a plausible dilemma specific to this role rather than a generic scenario; ` +
    `ask what a strong version of a named responsibility actually looks like in practice, not just "tell me ` +
    `about a time"; where sensible, ask a forward-looking "how would you approach..." question instead of a ` +
    `backward-looking STAR prompt, for variety. Vary your openers — don't make every question start with ` +
    `"Tell me about a time" or "Describe a time"; mix in things like "What would you do if...", "How would ` +
    `you decide...", "Walk me through how you'd...", "What's your view on...".\n\n` +
    `ALIGNMENT IS STILL THE WHOLE POINT: every question must trace back to something specific and real in the ` +
    `job description below — a named responsibility, a specific tool or process, a type of stakeholder, a ` +
    `named pressure or metric. Read the JD closely and pick ${count} DIFFERENT requirements to ground ` +
    `questions in (not the same theme repeated) — if the JD lists five distinct responsibilities, a candidate ` +
    `should be able to see each question map to a different one of them. Do not write a question that could ` +
    `just as easily apply to any unrelated job — if you can't tie it to something specific in this JD, don't ` +
    `include it. Do not write questions about the candidate's CV — you only have the job description, not ` +
    `their background.\n\n` +
